...
function mastLattice() {
  ...
  for (let y = MAST_TOP; y < MAST_FOOT; y += step) {
    ...
    out.push(
      <path
        key={`m${y}`}
        d={`M${left} ${y}L${right} ${y2}M${right} ${y}L${left} ${y2}`}
      />,
  ...
  for (let x = JIB_TIP; x < MAST_X - MAST_HALF; x += step) {
    ...
    out.push(
      <path
        ...
        d={
          up
            ? `M${x} ${JIB_BOT}L${x2} ${JIB_TOP}`
            : `M${x} ${JIB_TOP}L${x2} ${JIB_BOT}`
        }
      />,
  ...
  for (let x = MAST_X + MAST_HALF; x < CJIB_END; x += step) {
    ...
    out.push(
      <path
        ...
        d={
          up
            ? `M${x} ${CJIB_BOT}L${x2} ${CJIB_TOP}`
            : `M${x} ${CJIB_TOP}L${x2} ${CJIB_BOT}`
        }
      />,
    );
    up = !up;
...
export function CraneHero({
  ...
  return (
    /*
     * The pin. Without it the hook has nowhere to fall: the jib sits near the
     * top of the frame and disappears behind the header after roughly seventy
     * pixels of scroll, so the descent was over before it was visible. The
     * outer element is tall and the hero sticks to the top of it, which holds
     * the crane still while the page scrolls through — the hook then drops
     * against a stationary jib for most of a viewport.
     *
     * Whatever follows this must be positioned with a z-index, or the sticky
     * hero paints over it.
     */
    <div className="crane-hero-pin">
        ...
        <div className="crane-hero__art" aria-hidden="true">
          ...
          */}
            <g className="crane-haze">
              <rect
                className="haze haze--1"
                x={-HAZE_TILE}
                y="430"
                width={VB.w + HAZE_TILE * 2}
                height="240"
                fill="url(#hazeTile)"
                opacity="0.55"
              />
              <rect
                className="haze haze--2"
                x={-HAZE_TILE}
                y="580"
                width={VB.w + HAZE_TILE * 2}
                height="240"
            ...
            >
              <g className="crane-near">
                {/* Mast */}
                <path
                  d={`M${MAST_X - MAST_HALF} ${MAST_TOP}V${MAST_FOOT}`}
                  strokeWidth="3.4"
                />
                <path
                  d={`M${MAST_X + MAST_HALF} ${MAST_TOP}V${MAST_FOOT}`}
                ...
                <path
                  d={`M${MAST_X} ${CAT_APEX_Y + 6}L${JIB_TIP + 40} ${JIB_TOP - 2}`}
                  strokeWidth="1.9"
                />
                <path
                  d={`M${MAST_X} ${CAT_APEX_Y + 6}L${CJIB_END - 26} ${CJIB_TOP}`}
                  strokeWidth="1.9"
                />

                {/* Working jib */}
                <path
                  d={`M${JIB_TIP} ${JIB_TOP}H${MAST_X - MAST_HALF}`}
                  strokeWidth="3.2"
                />
                <path
                  d={`M${JIB_TIP} ${JIB_BOT}H${MAST_X - MAST_HALF}`}
                  strokeWidth="3.2"
                />
                ...
                {/* Counter-jib and counterweight */}
                <path
                  d={`M${MAST_X + MAST_HALF} ${CJIB_TOP}H${CJIB_END}`}
                  strokeWidth="3.2"
                />
                <path
                  d={`M${MAST_X + MAST_HALF} ${CJIB_BOT}H${CJIB_END}`}
                  strokeWidth="3.2"
                />
                {counterJibLattice()}
                <path
                ...
                <path
                  d={`M${MAST_X - MAST_HALF - 30} ${JIB_BOT + 4}h30v34h-30z`}
                />
